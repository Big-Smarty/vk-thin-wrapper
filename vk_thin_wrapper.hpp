...
using create_fn =
    std::conditional_t<std::is_same_v<ParentType, no_parent_t>,
                       ...
                       VkResult (*)(const ParentType *, const CreateInfo *,
                                    const VkAllocationCallbacks *, T *)>;
...
using destroy_fn =
    std::conditional_t<std::is_same_v<T, VkInstance> ||
                       ...
                       void (*)(ParentType, const VkAllocationCallbacks *, T)>;
template <typename T, typename CreateInfo, typename ParentType,
          create_fn<T, CreateInfo, ParentType> CreateFn,
          destroy_fn<T, ParentType> DestroyFn>
class thin_wrapper {
public:
  thin_wrapper(const CreateInfo &create_info) requires
      std::is_same_v<ParentType, no_parent_t> {
    try {
  ...
  thin_wrapper(const CreateInfo &create_info,
               const ParentType
                   &parent) requires(!std::is_same_v<ParentType, no_parent_t>)
      : m_parent(parent) {
    try {
  ...
  ~thin_wrapper() requires std::is_same_v<ParentType, no_parent_t> {
